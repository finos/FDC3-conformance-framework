...
export class ChannelControl2_0 implements ChannelControl<Channel, Context> {
  ...
  validateContextIsNotReceived = async(channel: Channel | null, errorMessage: string): Promise<void> => {
    if (channel) {
      listener1 = await channel.addContextListener(null, (context) => {
        assert.fail(errorMessage);
      });
    } else {
      listener1 = await fdc3.addContextListener(null, (context) => {
        assert.fail(errorMessage);
      });
    }
    validateListenerObject(listener1);
