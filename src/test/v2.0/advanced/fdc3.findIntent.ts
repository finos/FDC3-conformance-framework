...
export default () =>
  describe("fdc3.findIntent", () => {
    ...
    it("(2.0-FindIntentAppDByResultSingle) Should find intent 'cTestingIntent' belonging only to app intent-c with context 'testContextX' and result type 'testContextZ'", async () => {
      const appIntent = await fdc3.findIntent(Intent.cTestingIntent, { type: ContextType.testContextX }, ContextType.testContextZ);
      validateAppIntent(appIntent, 1, { name: Intent.cTestingIntent, displayName: "C Testing Intent" }, IntentApp.IntentAppC);
    });
