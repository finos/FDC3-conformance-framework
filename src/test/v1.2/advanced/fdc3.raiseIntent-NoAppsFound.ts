...
import { ContextType, IntentApp, Intent } from "./intent-support-1.2";
...
export default () =>
  describe("fdc3.raiseIntent (throws error)", () => {
    ...
    const test6 = "(FailedResolve2) Should fail to raise intent when targeted app intent-a (name and appId), context 'testContextY' and intent 'aTestingIntent' do not correlate";
    it(test6, async () => {
      try {
        await fdc3.raiseIntent(
          Intent.aTestingIntent,
    ...
    const test7 = "(FailedResolve3) Should fail to raise intent when targeted app intent-a (name), context 'testContextY' and intent 'aTestingIntent' do not correlate";
    it(test7, async () => {
      try {
        await fdc3.raiseIntent(
          Intent.aTestingIntent,
    ...
    const test8 = "(FailedResolve4) Should fail to raise intent when targeted app intent-c, context 'testContextX' and intent 'aTestingIntent' do not correlate";
    it(test8, async () => {
      try {
        await fdc3.raiseIntent(
          Intent.aTestingIntent,
